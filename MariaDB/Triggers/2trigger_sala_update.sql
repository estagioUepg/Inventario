...
BEGIN
    ...
    IF NEW.nome_n LIKE "%cca06%" OR NEW.nome_o LIKE "%cca06%"
    THEN 
    ...
    IF NEW.nome_n LIKE "%uepg-71%" OR NEW.nome_o LIKE "%uepg-71%"
    THEN 
    SET NEW.nome_sala = "CCA05";
    UPDATE sala SET qtd_maquinas = (select count(*) from maquina where nome_sala like "cca05") WHERE nome = "CCA05";
    END IF;
    
    IF NEW.nome_n LIKE "%lab06%" or NEW.nome_o LIKE "%lab06%"
    THEN 
